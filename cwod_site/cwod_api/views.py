...
class GenericHandler(BaseHandler):
    ...
    def read(self, request, *args, **kwargs):
        ...
        if 'date' in request.GET:
            ...
            defaults = (datetime.date(settings.START_YEAR, 01, 01).strftime('%Y-%m-%d'),
...
class PhraseByCategoryHandler(GenericHandler):
    ...
    def read(self, request, *args, **kwargs):
        ...
        if not request.GET.get('sort', 'count') in valid_sort_params:
            return {'error': 'Valid sort options for this endpoint are: %s. Sorting directions are not supported.' % ', '.join(valid_sort_params)}
    ...
    def read(self, request, *args, **kwargs):
        ...
        if not bioguide_id:
            ...
            for legislator in legislator_qs:
                current_congress = LegislatorRole.objects.all().values_list('congress', flat=True).distinct().order_by('-congress')[0]
